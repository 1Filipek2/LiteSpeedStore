...
class Snapshot {
public:
    static bool save(const std::string& path, const SnapshotImage& image); ///< Leaves existing snapshot untouched on failure.
    static bool load(const std::string& path, SnapshotImage& image);       ///< Returns false if absent, truncated, or CRC mismatch.
};
