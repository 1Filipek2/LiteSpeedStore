...
bool readLE64(const std::vector<uint8_t>& buf, size_t& offset, uint64_t& value) {
    if (offset + sizeof(uint64_t) > buf.size()) return false;
...
bool Snapshot::load(const std::string& path, SnapshotImage& image) {
    ...
    if (!readLE64(payload, offset, image.epoch)) return false;

    uint32_t keyCount = 0;
    if (!readLE32(payload, offset, keyCount)) return false;
    // a key entry needs at least MIN_KEY_BYTES on disk — reject impossible counts.
    if (keyCount > (payload.size() - offset) / MIN_KEY_BYTES) return false;

    image.state.clear();

    for (uint32_t i = 0; i < keyCount; ++i) {
        uint32_t keyLen = 0;
        if (!readLE32(payload, offset, keyLen)) return false;
        if (keyLen > payload.size() - offset) return false;

        std::string key(reinterpret_cast<const char*>(payload.data() + offset), keyLen);
        offset += keyLen;

        uint32_t recordCount = 0;
        if (!readLE32(payload, offset, recordCount)) return false;
        ...
        if (recordCount > (payload.size() - offset) / MIN_RECORD_BYTES) return false;
        ...
        for (uint32_t j = 0; j < recordCount; ++j) {
            ...
            if (!readLE64(payload, offset, rawTimestamp)) return false;
            if (!readLE64(payload, offset, rawDuration))  return false;
            rec.timestamp = static_cast<int64_t>(rawTimestamp);
            rec.duration  = bitsToDouble(rawDuration);

            uint32_t valueLen = 0;
            if (!readLE32(payload, offset, valueLen)) return false;
            if (valueLen > payload.size() - offset) return false;
